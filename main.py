...
def WriteWhoisInfo(DataBaseObject):
    ...
    while not (WhoisQueue.empty() and DomainQueue.empty()):  # 当双队列不空时候:
        if not WhoisQueue.empty():
            whois_dict = WhoisQueue.get()[0]
            WhoisInfoDict.Update(whois_dict)
        else:
            sleep(0.5)
...
def main(Worktype=0):
    ...
    DB.db_connect()
    # 填充域名队列
    SQL = """SELECT domain, flag FROM {DB}.{domainTable} """.format(
    ...
    update_whois_thread = threading.Thread(target=WriteWhoisInfo, args=[DB])
