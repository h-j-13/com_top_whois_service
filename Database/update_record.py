...
class WhoisRecord(object):
    ...
    def Update(self, whois_dict):
        ...
        print whois_dict
        __sql = """UPDATE """ + str(Static.DATABASE_NAME) + '.' + str(Static.DOMAIN_TABLE)
        __sql += """ SET domain = %s, flag = %s, \
        sponsoring_registrar = %s, sec_whois_server = %s, \
        name_server = %s, \
        details = %s """
        __values = (whois_dict['domain'], whois_dict['flag'], whois_dict['sponsoring_registrar'],
                    whois_dict['sec_whois_server'], whois_dict['name_server'], whois_dict['details'])

        if whois_dict['creation_date'] != '':
            __sql += """ ,creation_date = '{cd}' """.format(cd=whois_dict['creation_date'])

        if whois_dict['expiration_date'] != '':
            __sql += """ ,expiration_date = '{cd}' """.format(cd=whois_dict['expiration_date'])

        if whois_dict['updated_date'] != '':
            __sql += """ ,updated_date = '{cd}' """.format(cd=whois_dict['updated_date'])

        __sql += """WHERE domain = '{d}'""".format(d=whois_dict['domain'])

        self.DB.execute_sql_value(__sql, __values)
